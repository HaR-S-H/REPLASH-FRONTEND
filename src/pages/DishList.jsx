...
const DishList = () => {
  ...
  useEffect(() => {
    const fetchDishes = async () => {
        ...
        const response = await axios.get(
          ...
          fetchDishImages(response.data.data);
        setLoading(false);
      } catch (err) {
  ...
  const fetchDishImages = async (fetchedDishes) => {
    try {
      console.log(dishes);
      ...
      const updatedDishes = await Promise.all(
        fetchedDishes.map(async (dish) => {
          ...
          // If an image is found, attach it to the dish object
          if (imageResponse.data.results.length > 0) { 
            dish.imageUrl = imageResponse.data.results[0].urls.regular;
          } else {
            dish.imageUrl = "https://via.placeholder.com/300"; // Default image
          }
      ...
      setDishes(updatedDishes); // Update state with images
    } catch (error) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 flex justify-center items-center z-50">
          <div className="relative bg-white w-full max-w-lg max-h-[90vh] p-8 rounded-lg shadow-2xl overflow-y-auto transform transition-all duration-500 ease-in-out scale-105 opacity-100">
            ...
            <div className="space-y-4 text-lg text-gray-700">
              ...
              <pre>{selectedCard.recipe.replace(/-/g, "\n")}</pre>
